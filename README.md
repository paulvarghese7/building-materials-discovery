...
BuildMatch is a focused building-material product-discovery prototype created for a Full Stack Developer take-home assignment. It demonstrates how users can find a product when they know either its identity or its project requirement.
...
- **Product-first discovery:** search by product name, SKU, product type, feature, or description.
- **Requirement-first discovery:** browse and filter by acoustic, fire, or moisture requirements.
- **Transparent recovery:** explain typo interpretation and offer explicit ways to broaden an unsuccessful search.

The scope is deliberately small: 20 products, four product types, and three project requirements. This keeps the prototype deep enough to demonstrate product discovery without imitating an enterprise product-information system.

## Product highlights

- Responsive homepage, catalogue, filters, empty states, and product-detail routes.
- Shareable URL state through `q`, `type`, and `requirement` query parameters, with legacy URL compatibility.
...
- Contextual product-type and project-requirement counts that preserve the other active constraints.
- Human-readable match explanations on product cards.
- Linked product-type/project-requirement tags and same-type discovery from product details.
...
/products?type=boards&requirement=fire
/products?type=profiles&requirement=fire
...
| SKU | 100 |
| Product name | 80 |
| Product type / project requirement | 50 |
| Feature | 30 |
...
- **Flexible specifications:** `Specification[]` supports different product families without product-type-specific optional fields.
...
The project includes focused tests covering product data, URL parsing and compatibility, filtering, relevance ranking, typo recovery, contextual facets, suggestions, detail navigation, and empty-state recovery.
